...
const Map<String, Map<AppLanguage, String>> _tr = {
  ...
  'load_sample': {
    ...
    AppLanguage.ru: 'Загрузить пример',
    AppLanguage.kk: 'Үлгіні жүктеу',
  },
  'save_test': {
...
class _CreateTestPageState extends State<CreateTestPage> {
  ...
  void loadSample() {
    nameCtrl.text = 'AI in Inclusive Education';
    textCtrl.text = _sampleTestData;
    setState(() {});
  }

  static const _sampleTestData = r'''
<question> Жасанды Интеллектті инклюзивті білім беруге енгізудегі мұғалімнің рөлі қалай өзгереді?
<variant> Мұғалім – ЖИ-ді тиімді қолданушы, жекелендірілген оқытуды басқарушы болып қалады.
<variant> ЖИ мұғалімді толық алмастырып, оның рөлін жояды.
<variant> Мұғалім тек техникалық қолдаушының рөлін атқарады.
<variant> Мұғалімнің жұмысы еш өзгеріссіз қалады.
<variant> Мұғалімнің орнына ата-аналар оқытуды басқарады.

<question> Дыбыстық және визуалды ақпаратты бір уақытта ұсынатын бейімделген оқу материалдары қандай білім беру қағидатын іске асырады?
<variant> Әмбебап оқыту дизайны 
<variant> Стандарттау қағидаты.
<variant> Бағалауды шектеу қағидатын.
<variant> Тек бір бағытты оқыту қағидатын.
<variant> Арнайы мектепте оқыту қағидатын.

<question> Жасанды Интеллект инклюзивті білім беруде мұғалімдердің қай міндеттеріне көңіл бөлуге мүмкіндік береді?
<variant> Қайталанатын әкімшілік және бағалау жұмыстарынан.
<variant> Оқушылармен жеке қарым-қатынас жасаудан.
<variant> Эмоционалды қолдау көрсетуден.
<variant> Жаңа материалды түсіндіруден.
<variant> Кәсіби дамудан.

<question> Бейімделген тапсырмалар әзірлеуде Жасанды Интеллект (ЖИ) қандай негізгі функцияны орындайды?
<variant> Оқушының қателіктерін және оқу қарқынын талдап, қажеттілікке сәйкес өзгертеді.
<variant> Барлық оқушыларға арналған бірыңғай тест сұрақтарын автоматты түрде құрастырады.
<variant> Тек мұғалімдердің есеп беру құжаттарын толтырады.
<variant> Оқушылардың әлеуметтік дағдыларын бағалайды.
<variant> Мектептің интернет желісінің жылдамдығын реттейді.

<question> Инклюзивті оқытудағы бейімделген тапсырманың басты мақсаты не?
<variant> Әр оқушының оқу қажеттілігіне сәйкес материалды меңгеруге мүмкіндік беру.
<variant> Барлық оқушылардың бірдей уақытта бірдей нәтижеге жетуін қамтамасыз ету.
<variant> Ерекше білім беру қажеттіліктері бар оқушыларды жалпы сыныптан оқшаулау.
<variant> Қосымша материалдарды пайдалануды шектеу.
<variant> Мұғалімнің жұмыс жүктемесін міндетті түрде арттыру.

<question> Тілдік, танымдық деңгейіне қарай мәтіннің күрделілігін автоматты түрде реттейтін ЖИ қандай оқушыларға көмектеседі?
<variant> Оқу және тілдік қиындықтары бар оқушыларға.
<variant> Тек физикалық мүмкіндігі шектеулі оқушыларға.
<variant> Тек математика пәні бойынша.
<variant> Тек жоғары сынып оқушыларына.
<variant> Мұғалімдердің өзіне ғана.

<question> Цифрлық құралдарды қолданып, Универсалдық оқыту дизайны қағидаты бойынша бейімделген тапсырма әзірлеу нені талап етеді?
<variant> Ақпаратты ұсынудың баламалы тәсілдерін қамтамасыз ету.
<variant> Тек қана бір ұсыну форматын (мысалы, мәтін) пайдалану.
<variant> Тапсырманың күрделілігін бір деңгейде ұстау.
<variant> Цифрлық құралдарды мүлдем қолданбау.
<variant> Тек жоғары баға қоюды көздеу.

<question> Жасанды Интеллектке негізделген платформалар бейімделген тапсырмалардан кейін мұғалімге қандай маңызды деректерді ұсынады?
<variant> Әр оқушының жетістіктерінің жеке картасын.
<variant> Тек оқушының аты-жөнін.
<variant> Мектептің бюджеті туралы ақпаратты.
<variant> Басқа мектептердің нәтижелерін.
<variant> Мұғалімнің жалақысы туралы мәліметті.

<question> Бейімделген тапсырмалар әзірлеуде мультимедиялық элементтерді енгізудің маңызы неде?
 <variant> Бейімделу және ақпаратты меңгеру тиімділігін арттыру үшін.
<variant> Тек оқушылардың көңілін көтеру үшін.
<variant> Тапсырманы орындау уақытын ұзарту үшін.
<variant> Дәстүрлі оқулықтарды толықтай жою үшін.
<variant> Тек компьютерлік сауаттылықты тексеру үшін.
''';
                ...
                children: [
                  ...
                  Expanded(
                    child: OutlinedButton(
                      ...
                      child: Text(tr(context, 'load_sample')),
                    ),
                  ),
                ],
              ),
